  ...
  return (
    <div className="messagesInnerContainer">
      {getMessagesData.data
        ? [...getMessagesData.data.messagesList, ...realtimeMessages].map(
            message => {
              return <p className="p">{message.message}</p>;
            }
          )
        : "loading"}
      {getMessagesData.data &&
      [...getMessagesData.data.messagesList, ...realtimeMessages].length ==
        0 ? (
        <h3 className="h3">No Messages</h3>
      ) : (
        ""
      )}
    </div>
